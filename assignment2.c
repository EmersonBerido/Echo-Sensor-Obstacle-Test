...
int main() {
  ...
  // starts line sensor reading thread
  if (InitializeLineSensor() != 0) {
    printf("Failed to initialize line sensor\n");
    gpioTerminate();
    return 1;
  }
  if (StartLineSensorReading() != 0) {
    printf("Failed to start line sensor reading thread\n");
    gpioTerminate();
    return 1;
  }
  ...
  while (!exitRequested) {
    printf("Center Line: %d, Left Line: %d, Right Line: %d\n", centerLineValue, leftLineValue, rightLineValue);
